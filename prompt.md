...
- **By platform** — these ATS platforms have repeatedly turned out to be plain server-rendered (a real hit rate, not just a guess: Greenhouse, Lever, join.com, SmartRecruiters, and Ashby have ALL produced confirmed findings so far), so search them directly instead of only hoping generic queries surface one: `site:jobs.lever.co`, `site:job-boards.greenhouse.io`, `site:join.com`, `site:jobs.smartrecruiters.com`, `site:jobs.ashbyhq.com`, `site:*.teamtailor.com`, each combined with "Budapest" or "Hungary" plus an IT role word, for small companies (under ~20 open roles).
...
- `location` — the work location as stated on the posting (city name, "Budapest", "Remote", "Országosan", etc.), used by filter 6 above. Fill it whenever the posting states or implies one; leave it empty/omit only when truly unstated.
